...
const Slider = ({ heading, buttonheading, href }) => {
  ...
  useEffect(() => {
    let interval;
    if (!isHovered) { // Only auto-advance when not hovered
  ...
  }, [transitioning, isHovered]); // Add isHovered to dependencies
          ...
          >
            <div
              className={`flex ${transitioning
                  ? 'transition-transform duration-[2500ms] ease-[cubic-bezier(0.25,0.46,0.45,0.94)]'
                  : ''
                }`}
              style={{
            ...
            >
              {/* Rest of your slider content remains exactly the same */}
              {/* Slide 1 */}
              <div
